...
PROMO_TEXT = "IG glennesports7 | YT Glenn eSports"
...
if __name__ == "__main__":
    # Check if bot.txt exists (for backward compatibility)
    # If using Kivy app, this block won't run (app.py will be imported instead)
    if os.path.exists("bot.txt"):
        logging.info("Running in standalone mode with bot.txt")
        ...
        logging.warning("No bot.txt found. If running with Kivy app, this is normal.")
        logging.info("To run standalone, create bot.txt with format: {\"uid\": \"password\"}")
        logging.info("Or use the Kivy app (main.py) to select a .dat file.")
        
        # Keep the process alive for manual testing if needed
        try:
            while True:
                time.sleep(1)
        except KeyboardInterrupt:
